...
#pragma omp parallel for num_threads(10)
    for (int seed = 0; seed < num_seeds; seed++) {
        ...
        auto judge = std::make_shared<LocalJudge>(seed, -1, -1, 25);
...
#pragma omp critical(crit_sct)
        {
            ...
            std::cerr << format(
                "\rprogress=%3d/%3d, avg_score=%13.2f, avg_wrong=%5.2f, avg_placement=%13.2f, avg_measurement=%13.2f, min=%11lld, max=%11lld",
                progress, num_seeds, (double)score_sum / progress, (double)wrong_sum / progress, (double)placement_sum / progress, (double)measurement_sum / progress, min_score, max_score
            );
