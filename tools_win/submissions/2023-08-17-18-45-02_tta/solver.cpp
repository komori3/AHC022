...
struct LocalJudge : Judge {
    ...
    LocalJudge(const Input& input_, int seed = 0) : engine(seed), input(input_) {
        A.resize(input.N);
        std::iota(A.begin(), A.end(), 0);
        std::shuffle(A.begin(), A.end(), engine);
        std::normal_distribution<> dist_norm(0.0, input.S);
        F.resize(10000);
        for (int i = 0; i < 10000; i++) {
            F[i] = (int)round(dist_norm(engine));
        }
...
struct Params {
    ...
    Params(int num_quantize_, int intercept_, int slope_, int num_trial_, bool align_parity_, int displacement_)
        : bit_rate(num_quantize_), intercept(intercept_), slope(slope_), num_trial(num_trial_), align_parity(align_parity_), displacement(displacement_) {}
    std::string stringify() const {
        return format(
            "{num_quantize, intercept, slope, num_trial, align_parity, displacement} = {%d, %d, %d, %d, %d, %d}",
            bit_rate, intercept, slope, num_trial, align_parity, displacement
        );
    }
};
...
EncodedGridPtr find_unique_encoded_grid(
    ...
    std::vector<int> code_ctr((int)pow(bit_rate, num_neighbors));
    for (int i = 0; i < N; i++) {
        auto [y, x] = pos[i];
        int code = 0;
        for (int d = 0; d < num_neighbors; d++) {
...
Grid<int> create_temperature(
    ...
    const auto& displacements = egrid->displacements;
...
struct Sampler {
    ...
    Sampler(Judge& judge_, EncodedGridPtr egrid_, const Quantizer& quantizer, int id_) :
    ...
    bool sample() {
        ...
        for (int d = 0; d < num_neighbors; d++) {
            ctrs[d]++;
            sums[d] += judge.measure(id, egrid->displacements[d].y, egrid->displacements[d].x);
        }
        return true;
    }
